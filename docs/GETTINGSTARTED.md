...
1. **Set your token in the ADO_MCP_AUTH_TOKEN environment variable:**
   ...
   ```bash
   export ADO_MCP_AUTH_TOKEN="your-nholuongut-token"
   ```
   ...
   ```json
   {
     ...
     "servers": {
       "ado": {
         "type": "stdio",
         "command": "npx",
         "args": ["-y", "@nholuongut/mcp", "${input:ado_org}", "--authentication", "envvar"]
       }
     }
   }
   ```
...
```bash
claude mcp add nholuongut -- npx -y @nholuongut/mcp Contoso
```
